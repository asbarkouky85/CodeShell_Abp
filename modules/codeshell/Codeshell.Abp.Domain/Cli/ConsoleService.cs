...
{
    public class ConsoleService : DomainService
    {
        ...
        public void WriteException(Exception ex, bool full = true)
        {
            ...
            if (ex is RemoteServerException)
            {
                var csEx = ex as RemoteServerException;
                if (csEx.RemoteResult != null)
                {
                    WriteResult(csEx.RemoteResult);
                    return;
                }
                else if (ex.Message.TryRead(out RemoteServerResult res))
                {
                    WriteResult(res);
                    return;
                }
