...
{
    internal class CliRequestBuilder<T> : ICliRequestBuilder<T> where T : class
    {
        private List<CliArgument<T>> _keys = new List<CliArgument<T>>();

        public IEnumerable<CliArgument<T>> KeyList => _keys;

        public CliArgument<T, TVal> Property<TVal>(Expression<Func<T, TVal>> t, string key, string? shortKey = null, int? order = null, bool isRequired = false) where TVal : class
        {
            var item = new CliArgument<T, TVal>(t, key, shortKey, order, isRequired);
            _keys.Add(item);
            return item;
        }
